...
def motor_turn(text,up_string,down_string):
    ...
    #this function turns a motor to position 1 (0 degrees)
    #if 'up_string' is found in the text descriptions returned
    #using regular expressions - that's re.search(x,y,__) which
    #looks for string x in string y. z, '___' here is set to
    #re.IGNORECASE, which searches in a case-insensitive fashion.
    #similarily, if 'down_string' is found, the motor moves to position 2. 
    
    if re.search(up_string, text, re.IGNORECASE):
